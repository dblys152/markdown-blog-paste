...
from md2blog.modules.identity.infrastructure.models import EmailVerificationTokenModel
...
class SqlAlchemyEmailVerificationTokenRepository:
    ...
    async def find_latest_by_user_id(self, user_id: TSID) -> EmailVerificationToken | None:
        statement = (
            select(EmailVerificationTokenModel)
            .where(EmailVerificationTokenModel.user_id == user_id.value)
    ...
    async def save(self, token: EmailVerificationToken) -> None:
        model = await self._session.get(EmailVerificationTokenModel, token.id.value)
        if model is None:
            raise LookupError("email verification token not found")
    ...
    async def count_created_since(self, user_id: TSID, since: datetime) -> int:
        statement = select(func.count()).select_from(EmailVerificationTokenModel).where(
            EmailVerificationTokenModel.user_id == user_id.value,
            EmailVerificationTokenModel.created_at >= since,
        )
    ...
    @staticmethod
    def _to_domain(model: EmailVerificationTokenModel) -> EmailVerificationToken:
