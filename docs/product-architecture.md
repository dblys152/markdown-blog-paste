...
이미 적용되었거나 다른 주체가 소비한 변경 이력과 계약은 불변으로 취급합니다. 이후
변경은 과거 정의를 소급 수정하지 않고 새 마이그레이션, 새 계약 버전, 호환 계층 또는
명시적인 데이터 변환으로 표현합니다.

- DB 마이그레이션은 적용 순서를 나타내는 이력이므로 적용된 revision을 수정하지 않습니다.
- 도메인 이벤트와 Outbox 메시지는 발행 이후 생산자와 소비자 사이의 계약이 됩니다.
- API와 저장 포맷 변경은 기존 소비자 및 저장 데이터와의 호환성을 함께 검토합니다.
- 이력 식별자가 최신인 상태와 실제 데이터·스키마가 계약에 부합하는 상태를 구분합니다.

이 원칙은 과거 상태를 임의로 다시 해석하는 위험을 줄이는 대신, 후속 변환과 호환성
유지 비용을 요구합니다.
...
| 이력과 계약의 진화 | 적용·발행된 정의를 불변으로 취급 | DB, 이벤트, 메시지와 API의 소비 상태를 보존하고 환경 간 불일치를 방지합니다. | 변경 시 새 버전과 명시적인 변환 경로를 관리해야 합니다. |
