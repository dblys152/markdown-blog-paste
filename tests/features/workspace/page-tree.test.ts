...
const pages: WorkspacePage[] = [
  { id: "1", title: "A", content: "", parent_id: null, position: 0 },
  { id: "2", title: "B", content: "", parent_id: null, position: 1 },
  { id: "3", title: "C", content: "", parent_id: "1", position: 0 },
];
...
describe("page tree move", () => {
  ...
  it("이동 후 이전 계층과 새 계층의 순서를 다시 매긴다", () => {
    const moved = applyPageMove(pages, "2", { parentId: "1", position: 0 });

    expect(moved.find((page) => page.id === "2")).toMatchObject({ parent_id: "1", position: 0 });
    expect(moved.find((page) => page.id === "3")).toMatchObject({ parent_id: "1", position: 1 });
    expect(moved.find((page) => page.id === "1")).toMatchObject({ parent_id: null, position: 0 });
  });
});
